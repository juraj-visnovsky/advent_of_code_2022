...
class Location {
  ...
  constructor(value: string, x: number, y: number) {
    ...
    this.y = y;
    this.value = value;
    this.isStart = value === "E";
    this.isDestination = value === "S" || value === "a";
  ...
  private calculateHeight() {
    ...
    if (this.isStart) value = "z";
    if (this.isDestination) value = "a";

    return value.charCodeAt(0) - 96;
  }
}

export const findShortestPathDown = (input: string): number => {
...
const locationIsReachable = (currentLocation: Location, grid: Grid, nextX: number, nextY: number) => {
  ...
  const isReachable = isOnGrid && currentLocation.height <= grid[nextX][nextY].height + 1;
